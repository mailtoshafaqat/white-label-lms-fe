...
| 2026-06-09 | File storage | **Multi-provider `IFileStorage`:** `LocalDiskFileStorage`, `R2FileStorage`, `AzureBlobFileStorage` selected by `FileStorage.Provider` in appsettings (DI only; no feature code changes) |
| 2026-06-09 | Enrollment | **Flashcards gated** for students via `IEnrollmentAccessGuard` on `GET /topics/{id}/flashcards` |
| 2026-06-09 | Payments | **Manual payment admin email:** `ManualPaymentSubmittedEvent` → `ManualPaymentSubmittedHandler` emails institute admins (requires tenant SMTP) |
