...
export default function LandingSettingsPage() {
  ...
  async function uploadHeroImage(key: string, file: File) {
    setUploadingHeroKey(key);
    setError(null);
    try {
      const result = await adminApi.uploadFile(file, "landing");
      updateHero(key, { imageUrl: result.url });
    } catch (err) {
  ...
  function previewDraftLanding() {
    const slug = getSession()?.tenant?.slug ?? getTenantSlug();
    saveLandingPreview(slug, {
      slug,
      sections: sections
    ...
    openPreview("/", slug, {
      landing: true,
      branding: !!loadBrandingPreview(slug),
    });
  }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      const page = await adminApi.saveLanding({ sections: payload });
      setSections(
        page.sections.map((s) => ({
          ...s,
          key: s.id ?? newKey(),
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-3xl px-6 py-8">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <Button type="button" variant="outline" onClick={previewDraftLanding}>
            <ExternalLink className="h-4 w-4" /> Preview draft
          </Button>
        </div>
                ...
                <Card key={s.key}>
                  ...
                  <CardContent className="space-y-3">
                    {hero && (
                      <>
                        <div>
                          <label className={label}>Hero banner image (optional)</label>
                          <p className="mb-2 text-xs text-slate-500">
                            One wide banner for the top of your homepage. Recommended 1600×600 px or
                            similar landscape ratio.
                          </p>
                          <div className="flex flex-wrap items-center gap-3">
                            {resolveAssetUrl(hero.imageUrl ?? null) && (
                              <div className="relative">
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                <img
                                  src={resolveAssetUrl(hero.imageUrl ?? null)!}
                                ...
                                <button
                                  ...
                                  onClick={() => updateHero(s.key, { imageUrl: null })}
                            ...
                            <label className="inline-flex cursor-pointer items-center gap-2 rounded-md border border-slate-300 bg-white px-3 py-2 text-sm hover:bg-slate-50">
                              ...
                              {uploadingHeroKey === s.key ? "Uploading…" : "Upload banner"}
                              <input
                                type="file"
                                accept="image/png,image/jpeg,image/webp,image/gif"
                                className="hidden"
                                disabled={uploadingHeroKey === s.key}
                                onChange={(e) => {
                                  const file = e.target.files?.[0];
                                  if (file) void uploadHeroImage(s.key, file);
                                  e.target.value = "";
                                }}
                              />
                            </label>
