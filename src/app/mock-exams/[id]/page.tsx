...
import type { NavigatorSection } from "@/components/assessment/question-navigator";
import {
  mockExamsApi,
  type MockExamAttemptResultDto,
  type MockExamQuestionDto,
  type MockExamSectionNavDto,
  type MockExamSummaryDto,
} from "@/lib/api";
import { getSession } from "@/lib/auth";
import { toggleFlagged } from "@/lib/quiz-attempt";
...
export default function MockExamAttemptPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [sections, setSections] = useState<MockExamSectionNavDto[]>([]);
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      setResult(
        await mockExamsApi.submit(id, payload, attemptId, Array.from(flagged))
      );
    } catch (e) {
  ...
  }, [id, attemptId, questions, answers, flagged]);
  ...
  const navigatorSections: NavigatorSection[] = sections.map((s) => ({
    title: s.title,
    startIndex: s.startQuestionNumber - 1,
      ...
      <main className="mx-auto max-w-5xl px-4 py-8">
        ...
        {result ? (
              ...
              <>
                <Card className="mb-6">
                  <CardContent className="space-y-2 pt-6 text-center">
                    <p className="text-4xl font-bold text-[var(--brand)]">{result.score}</p>
                    <p className="text-sm text-slate-600">
                      {result.correctCount} correct · {result.wrongCount} wrong · {result.total}{" "}
                      questions
                    </p>
                    <p className="text-xs text-slate-500">
                      Marking: +{result.marksPerCorrect} / −{result.penaltyPerWrong}
                    </p>
                    {result.rank && (
                      <p className="text-sm font-medium text-slate-800">
                        Rank {result.rank.rank} of {result.rank.totalAttempts} (
                        {result.rank.percentile}th percentile)
                      </p>
                    )}
                  </CardContent>
                </Card>
          ...
          <McqAttemptView
            ...
            flagged={flagged}
            onToggleFlag={(questionId) => setFlagged((prev) => toggleFlagged(prev, questionId))}
          ...
          <Card>
            <CardContent className="space-y-3 pt-6 text-center">
              <p className="text-slate-600">
                ...
                {exam.marksPerCorrect}/−{exam.penaltyPerWrong} marking
              </p>
